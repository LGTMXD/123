...
downloadV2Ray(){
    rm -rf /tmp/v2ray
    mkdir -p /tmp/v2ray
    if [[ "${DIST_SRC}" == "lgtmxd" ]]; then
        DOWNLOAD_LINK="lgtmxd.com/v2ray-linux-${VDIS}.zip"
    else
        DOWNLOAD_LINK="lgtmxd.com/v2ray-linux-${VDIS}.zip"
    fi
...
# 1: new V2Ray. 0: no. 2: not installed. 3: check failed. 4: don't check.  deleted by lgtmxd
